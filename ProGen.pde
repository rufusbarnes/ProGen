...
float nS = 0;
Textfield nSInput;
Button nSButton, eButton, iButton;

//PrintWriter to export settings
PrintWriter output;

//Required to set icon
public void settings() {
  //Set App Icon
  PJOGL.setIcon("Icon5.png");
  size(600, 800, "processing.opengl.PGraphics3D");
}
...
void setup() {
  ...
  eButton = control.addButton("Export")
                   .setSize(49, 20)
                   .setPosition(10, 225);
  iButton = control.addButton("Import")
                   .setSize(49, 20)
                   .setPosition(61, 225);
...
void Import() {
  ...
  //Terrain Layers
  control.getController("t1HR").setValue(int(settings[0]));
  control.getController("t1NI").setValue(int(settings[1]));
  control.getController("t2HR").setValue(int(settings[2]));
  control.getController("t2NI").setValue(int(settings[3]));
  control.getController("t3HR").setValue(int(settings[4]));
  control.getController("t3NI").setValue(int(settings[5]));
